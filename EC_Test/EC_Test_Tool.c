...
/* Copyright (C)Copyright 2005-2020 zxq Telecom. All rights reserved.
   ...
   Description:These functions of this file are reference only in the Linux!
   It can read/write ITE-EC RAM by 
   ----PM-port(62/66)
   ----KBC-port(60/64)
   ----EC-port(2E/2F or 4E/4F)
   
   This is ITE EC test tool in the Linux system.
   Compiler ： gcc EC_Teset_Tool.c
*/
...
UINT32 IO_device_init(void)
{
	...
	printf("device_init: open IOPORT file successful!\n");
	return 1;
}

void IO_device_Close(void)
{
    fclose(fd);
}

void iowrite(UINT8 io_port, UINT8 data)
{
	fseek(fd, io_port, SEEK_SET);
	
    fwrite(&data, 1, 1, fd);
}	

void ioread(UINT8 io_port, UINT8 *data)
{
	fseek(fd, io_port, SEEK_SET);
...
#define READ_PORT(port,data2)  ioread(port, &data2)
#define WRITE_PORT(port,data2) iowrite(port, data2)
...
UINT8 ECRamReadExt_Direct(UINT16 iIndex)
{
    UINT8 data;
...
void ECRamWriteExt_Direct(UINT16 iIndex, BYTE data)
{
    UINT8 data1;
    data1 = data;
    WRITE_PORT(EC_ADDR_PORT, 0x2E);
...
int main(int Argc, char *Argv[])
{
    ...
    // Init IO
    IOInitOK = IO_device_init();
    if(0 == IOInitOK)
    {
        printf("IO device open Error\n");
        goto IOError;
    }
